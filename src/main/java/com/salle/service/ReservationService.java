...
public class ReservationService {
    ...
    public Reservation createReservation(Long userId, Long salleId, LocalDateTime dateDebut,
        ...
        // Check if user already has a reservation during this period (any room)
        List<Reservation> userConflicts = reservationDAO.getReservationsByUserAndPeriod(userId, dateDebut, dateFin);
        if (!userConflicts.isEmpty()) {
            throw new OverlapException("Vous avez déjà une réservation pour cette période.");
        }
        ...
        if (!overlapping.isEmpty()) {
            ...
            // Optionally, return a specific reservation status or throw a custom exception
            // for waiting list
            // For now, we'll just return null or a placeholder reservation indicating
            // waiting list entry
            // This part might need refinement based on how the UI should react
            System.out.println(
                    "Salle " + salle.getNom() + " is full. User " + user.getEmail() + " added to waiting list.");
            throw new OverlapException(
                    "Cette salle est déjà réservée pour cette période. Vous avez été ajouté à la liste d'attente.");
        }
    ...
    public Reservation updateReservation(Long reservationId, Long userId, Long salleId,
        ...
        // Check if user already has another reservation during this period (any room,
        // excluding current)
        List<Reservation> userConflicts = reservationDAO.getReservationsByUserAndPeriod(userId, dateDebut, dateFin);
        ...
        // Check for overlapping reservations in the same room (excluding current
        // reservation)
    ...
    public String cancelReservation(Long reservationId, Long userId)
        ...
        if (autoAssigned) {
            ...
            notificationService.createNotification(userId,
                    "Votre réservation pour la salle " + reservation.getSalle().getNom()
                            + " a été annulée. Une salle a été attribuée à un utilisateur en attente.",
